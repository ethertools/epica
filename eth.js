...
var getStaticMethods = function(){
  return {
    ...
    newBlock: function(f) {
      ...
      m_watching[""] = f;
      f();
      if (isEmpty(m_watching) != old) this.check();
    }
  };
};

var getRpcMethods = function(opt){
  return _.reduce(spec, function(memo,item){
    memo["get" + item.method[0].toUpperCase() + item.method.slice(1)] = function(){
      var args = arguments;
      return request.post({
        url: opt.host || "http://localhost:8080",
        json: {
          jsonrpc: "2.0",
          method: item.method,
...
module.exports = function(opt){
  ...
  return _.extend({}, getStaticMethods(),getRpcMethods(opt));
};
